...
//  #pragma omp parallel for shared(row, col, array) private(i)
  for(i=0; i<row; i++){
...
void array_zero(int row, int col, int*** array){
  int i,j;
  #pragma omp parallel for shared(row, col, array) private(i, j)
  for(i=0; i<row; i++){
    for(j=0; j<col; j++){
      (*array)[i][j] = 0;
    }
...
int main(int argc, char *argv[]){
  ...
  if(argc != 7){
    ...
    printf("Konstrukcja: program wielkosc_macierzy ilosc_krokow plik_danych(sama nazwa) animacja(0-nie, 1-tak) wynik_do_pliku(0-nie, 1-tak) czas_do_pliku(0-nie, 1-tak)\n");
    exit(1);
  }
...
//  clock_gettime( CLOCK_REALTIME, &start2);
  ...
  for(counter = 1; counter <= a_steps; counter++){
    if(last_used == 1){
      #pragma omp parallel for shared(N, M, arr1, arr2) private(i, j, area_amount)
      for(i=0; i<N; i++){
        for(j=0; j<M; j++){
